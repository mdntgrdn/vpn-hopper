#!/usr/bin/env python3
"""Deploy the vless-grpc wrapper on the chain entry host (runs last, separately).
...
def deploy_wrapper(chain: str, *, generate: bool = True) -> int:
    ...
    if rc == 0:
        print(f"vless-grpc wrapper deployed on {host!r}.")
    return rc
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap = argparse.ArgumentParser(description="Deploy the vless-grpc wrapper for a chain")
    add_chain_cli(ap)
