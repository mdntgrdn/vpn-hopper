#!/usr/bin/env python3
"""Remove vless-grpc client(s) from the wrapper host: rmu, drop from registry."""
...
from scripts.lib.vless_grpc.client_export import _export_stem
...
def remove_clients(chain: str, names: list[str], *, generate: bool = True) -> int:
    ...
    if missing:
        for name in missing:
            print(f"No vless-grpc client named {name!r} on wrapper {host!r}.", file=sys.stderr)
        return 1
    ...
    print(f"Removed {len(matched)} vless-grpc client(s) from {host!r}.")
    return 0
