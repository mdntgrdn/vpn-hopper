#!/usr/bin/env python3
"""Read/format the vless-grpc client registry (vless_clients.yml).

The registry lives on the wrapper host under <deploy_dir>/vless-grpc/ and is
...
VLESS_CLIENTS_KEY = "vless_grpc_clients"
...
def format_table(host: str, rows: list[dict]) -> str:
    lines = [f"wrapper: {host}", f"vless-grpc clients: {len(rows)}", ""]
    if not rows:
        lines.append("(empty)")
        return "\n".join(lines) + "\n"
