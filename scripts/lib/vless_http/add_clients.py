#!/usr/bin/env python3
"""Add vless-http client(s) on the wrapper host: append registry, render, hot-add."""
...
from scripts.lib.vless_http.build_adu import write_adu_payload
from scripts.lib.vless_http.client_batch import build_vless_peer_batch
...
def _validate_names(batch: list[dict], existing: list[dict], host: str) -> list[str]:
    ...
    for peer in batch:
        ...
        if find_username_row(name, existing) is not None:
            errors.append(
                f"Cannot add {name!r}: a vless-http client with that name already "
...
def add_clients(chain: str, names: list[str], *, generate: bool = True) -> int:
    ...
    write_adu_payload(paths.vless_http_configs, host, listen_port=listen_port, batch=batch)
    ...
    for peer in batch:
        ...
        print(f"Client {peer['username']!r} (vless-http)\n  {out}\n")

    cleanup_work_dir(paths)
    print(f"Added {len(batch)} vless-http client(s) on {host!r}.")
    return 0
