#!/usr/bin/env python3
"""Fetch the vless-http client registry from the wrapper host and list it."""
...
from scripts.lib.vless_http.clients_registry import (
    clients_for_host,
    format_output,
    load_registry,
)
...
def fetch_clients(chain: str, *, generate: bool = True) -> tuple[str, list[dict]]:
    ...
    rc = run_vless_http_playbook(FETCH_PLAYBOOK, chain, extra_vars={"vless_wrapper_host": host})
    if rc != 0:
